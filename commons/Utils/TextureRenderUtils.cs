                ...
                if (uidynamicFont.baseFont.GetCharacterInfo(text[i], out CharacterInfo glyph, Mathf.CeilToInt(size), style))
                {
                    ...
                    if (outlineColor != default)
                    {
                        for (int j = 0; j < kOutlineOffsets.Length; j++)
                        {
                            ...
                            MergeTextures(tex, colors.Select(z => new Color(outlineColor.r, outlineColor.g, outlineColor.b, z.a)).ToArray(), Mathf.RoundToInt(targetOffset.x), Mathf.RoundToInt(targetOffset.y), glyph.glyphWidth, glyph.glyphHeight, glyph.uvTopRight.y > glyph.uvTopLeft.y, glyph.uvBottomLeft.y > glyph.uvTopLeft.y, glyph.uvBottomRight.x < glyph.uvBottomLeft.x, true);
                        }
                    }


                    MergeTextures(tex, colors.Select(z => new Color(textColor.r, textColor.g, textColor.b, z.a)).ToArray(), Mathf.RoundToInt(vector4.x), Mathf.RoundToInt(vector4.y), glyph.glyphWidth, glyph.glyphHeight, glyph.uvTopRight.y > glyph.uvTopLeft.y, glyph.uvBottomLeft.y > glyph.uvTopLeft.y, glyph.uvBottomRight.x < glyph.uvBottomLeft.x);
                    x += glyph.maxX;
                }
            }
            return x;
